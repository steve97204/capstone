...
if __name__ == '__main__':
    ...
    for i, num in enumerate(["grad", "momentum","acc","error","weight", "activate"]):
        num_rounding = getattr(args, "{}_rounding".format(num))
        num_man = getattr(args, "{}_man".format(num))
        num_exp = getattr(args, "{}_exp".format(num))
        number_dict[num] = BlockFloatingPointWrapper(exp=num_exp, man=num_man)

        print("{:10}: {}".format(num, number_dict[num]))
    #minifloat
    '''for i, num in enumerate(["weight", "activate"]):
        num_rounding = getattr(args, "{}_rounding".format(num))
        num_man = getattr(args, "{}_man".format(num))
        num_exp = getattr(args, "{}_exp".format(num))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_accuracies.append(train_res['accuracy'])
        if epoch == 0 or epoch % args.eval_freq == args.eval_freq - 1 or epoch == args.epochs - 1:
            ...
            test_accuracies.append(test_res['accuracy'])
            test_accuracies_epochs.append(epoch + 1)
        else:
    ...
    plt.plot(test_accuracies_epochs, test_accuracies, label='Testing Accuracy')

    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title('Training and Testing Accuracy vs Epoch')
    plt.legend()
    plt.savefig(os.path.join(dir_name, 'accuracy_vs_epoch.png'))
    plt.show()
